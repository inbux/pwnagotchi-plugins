...
class WpaSec(plugins.Plugin):
    ...
    def _download_from_wpasec(self, output, timeout=30):
        ...
        try:
            ...
            with open(output, 'r') as cracked_list:
                for row in csv.DictReader(cracked_list, fieldnames=("BSSID", "mac", "ESSID", "password"), delimiter=":"):
                  
                   filename = re.sub(r'[^a-zA-Z0-9]', '', row['ESSID']) + '_' + row['BSSID'].replace(':','')
                   
                   if os.path.exists( os.path.join(self.handshake_dir, filename+'.pcap') ):
                       with open(os.path.join(self.handshake_dir, filename+'.pcap.cracked'), 'w') as f:
    ...
    def on_internet_available(self, agent):
        ...
        with self.lock:
            ...
            handshake_filenames = os.listdir(self.handshake_dir)
            handshake_paths = [os.path.join(self.handshake_dir, filename) for filename in handshake_filenames if
            ...
            if 'download_results' in self.options and self.options['download_results']:
                cracked_file = os.path.join(self.handshake_dir, 'wpa-sec.cracked.potfile')
                if os.path.exists(cracked_file):
                    last_check = datetime.fromtimestamp(os.path.getmtime(cracked_file))
                    if last_check is not None and ((datetime.now() - last_check).seconds / (60 * 60)) < 1:
                        return
                try:
                    self._download_from_wpasec(os.path.join(self.handshake_dir, 'wpa-sec.cracked.potfile'))
